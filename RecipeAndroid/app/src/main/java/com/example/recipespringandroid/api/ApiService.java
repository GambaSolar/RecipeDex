...
public interface ApiService {
    ...
    @POST("recipes")
    Call<Recipe> createRecipe(@Query("userId") int userId,
                              @Body Recipe recipe);
    ...
    Call<Void> addIngredientToRecipe(@Query("recipeId") int recipeId,
                                     @Query("ingredientId") int ingredientId);

    @GET("recipe-ingredients/ingredient/{ingredientId}")
    Call<List<Recipe>> getRecipesByIngredient(@Path("ingredientId") int ingredientId);

    @POST("reviews")
    Call<Review> addReview(@Query("userId") int userId,
                           @Query("recipeId") int recipeId,
                           @Body Review review);

    @GET("reviews/recipe/{recipeId}")
    Call<List<Review>> getReviewsByRecipe(@Path("recipeId") int recipeId);

    @GET("favorites/{userId}")
    Call<List<Recipe>> getFavorites(@Path("userId") int userId);

    @POST("favorites")
    Call<Void> addFavorite(@Query("userId") int userId,
                           @Query("recipeId") int recipeId);
}